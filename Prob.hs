...
pascal n k = product [k + 1 .. n] `div` product [1 .. n - k]
comb n k = if k >= 0 then product [n - k + 1 .. n] `div` product [1 .. k] else 0 -- this is the same as pascal above!
...
binomial n p k = toRational (pascal n k) * p ^ k * (1 - p) ^ (n - k)
