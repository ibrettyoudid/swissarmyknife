...
commonPrefix a b n = if a == b then B.shift (a * 2 + 1) (n - 1) else commonPrefix (div a 2) (div b 2) (n + 1)

log2 k = ceiling $ logBase 2 $ fromIntegral k

nextKey k = (k+) $ B.shift 1 $ fromJust $ L.findIndex (\x -> x B..&. 1 == 1) $ iterate (`B.shift` -1) k

prevKey k = (k-) $ B.shift 1 $ lsbit k
