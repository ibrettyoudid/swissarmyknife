...
data Term = Term { tat :: [Double], tselected :: Bool, ttype :: TermType }

data Wire = Wire { wterms :: [Term], wselected :: Bool }

data Box = Box { at :: [[Double]], selected :: Bool, text :: String, terms :: [Term], wires :: [Wire], boxes :: [Box] }
...
main = do
   ...
   let
      ...
      drawTerm t = do
         ...
         rect [tat t <-> [3, 3], tat t <+> [3, 3]]
         fill
      drawWire w = do
         colourSel $ wselected w
         let [wt0, wt1] = wterms w
         newPath
         movTo $ tat wt0
         linTo $ tat wt1
         stroke
      ...
      hitBox hitAt box = let
         terms1 = map (hitTerm hitAt) $ terms box
         wires1 = map (hitWire hitAt) $ wires box
         boxes1 = map (hitBox  hitAt) $ boxes box
         in box
         {
            terms = terms1,
            wires = wires1,
            boxes = boxes1,
            selected = not (any tselected terms1 || any wselected wires1 || any selected boxes1) && hitAt `inRect` at box
         }

      hitTerm hat term = term { tselected = hat `inRect` [tat term <-> [5, 5], tat term <+> [5, 5]] }
      hitWire hat wire = let
         [wt0, wt1] = wterms wire
         wl = tat wt1 <-> tat wt0
         wll = modulus wl
         wlu@[wlx, wly] = unit wl
         ...
         in if wrl >= 0 && wrl < wll && abs wrw < 5
               then if wrl / wll < 0.5
                        then wire { wterms = [wt0 { tselected = True  }, wt1 { tselected = False }], wselected = True }
                        else wire { wterms = [wt0 { tselected = False }, wt1 { tselected = True  }], wselected = True }
               else
                  wire { wterms = [wt0 { tselected = False }, wt1 { tselected = False }], wselected = False }
