...
getWikiTables m title = filterWikiTables $ getNested m $ wiki title
...
area m = M.fromList $ map (\row -> (extractText $ findType "a" $ row !! 1, readInt $ extractText $ row !! 2)) $ drop 1 $ cGridH $ getWikiTable m "List of countries and dependencies by area"
...
mileq m = by (? "country") $ 
   ...
   map (\row -> 
      ...
      (map (toDyn . readInt . extractText) $ init $ drop 3 row)) $ drop 1 $ wikiGrid m "List of countries by level of military equipment"
...
text1 t = let
   as = findTypes "a" t
   in if null as then extractText t else extractText $ head as
...
allies3 m = let
   ...
   in Table (M.fromList $ zip ["country"] [0..]) $ 
         INode $ M.fromList $ 
         map (\row -> (
            toDyn $ text1 $ row !! 1, 
            Recs $ tz $ map (Rec . tz . singleton . toDyn . extractText) $ findTypes "a" $ row !! 2)) g

allies4 m = let
   g = padRWith (Text "") $ concatMap cGridH $ init $ getWikiTables m "List of military alliances"

   in fromGrid1 ["group", "country"] $ 
   concatMap (\row -> map (\country -> [toDyn $ text1 $ row !! 1, toDyn $ extractText country]) $ 
      findTypes "a" $ row !! 2) g

allies5 m = let
   g = padRWith (Text "") $ concatMap cGridH $ init $ getWikiTables m "List of military alliances"

   in byl [(? "group"), (? "country")] $ fromGrid1 ["group", "country"] $ 
...
cbr m = let
   ...
   s = map sum t
   in (crimes:) $ zipWith (:) races $ map2 (show . round . (/0.66)) $ Data.List.transpose t -- $ zipWith (\s c -> map (/ s) c) s t
...
nobels m = let
   ...
   in map (counts . concatMap (map (nationality m . wiki) . extractLinks)) $ drop 1 t
...
writeCsv file grid = writeFileBinary file $ Https.bsofs $ unlines $ map (intercalate "," . map quote) grid
...
cjpage m tm url = do
   ...
   let gs3 = if not (null gs2) && allEqual (map fst gs2) && allDiff (map (map fst . M.toList . snd) gs2)
...
cjgrid1 u g = let
   tg = cTextGridH g
   in ifJust (not $ classCon "mw-collapsed" g) $ tmofl1 (last $ split "/" u) tg
...
cjgridA1 u g = let
   tg = cTextGridH g
   in ifJust (not $ classCon "mw-collapsed" g) $ fromGridHD u tg

cjgridA2 tm1 g2 = do
   let res = Table1G.join (toDyn "") tm1 g2
...
fromElems ifcs = let
   ...
   in (fs, M.fromList $ map (\i -> (i, map (\f -> fromMaybe "" $ M.lookup (i, f) ifcs) fs)) is)

canMerge a b = isJust $ canMerge1 a b

canMerge1 a@(afs, ars) b@(bfs, brs) = let
   ifcs  = toElems b
   afm   = M.fromList $ zip afs [0..]
   found = map (\((i, f), c) -> (ars M.! i) !! (afm M.! f)) ifcs
   in ifJust (not (allDiff $ nubSet $ afs ++ bfs) && all null found) (afm, ifcs)
                 
merge a b = case canMerge1 a b of
   Nothing -> joinTMapsFuzzy "" b a
   Just c  -> Main.merge1 c a
