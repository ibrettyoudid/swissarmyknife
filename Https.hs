...
isCont (Tag name _ _) = let n = map toLower name in n /= "p" && n /= "br" -- S.member (map toLower name) tagSet
isCont x = False
...
htmlP = many (try comment <|> tag <|> text)
...
imdb1 m = cGrid $ subTag $ subTag $ findId "main" $ getNested m "http://www.imdb.com/chart/2000s"
