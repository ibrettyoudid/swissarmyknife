...
class MVR a b c where
   conr :: a -> b
   desr :: b -> a

class MVC a b c where
   conc :: a -> b
   desc :: b -> a
{-
class (MVC a c, MVC b d) => MVC2 a b c d where
mapcols :: (a -> b) -> c -> d
-}
instance Num a => MVR [[a]] [[a]] a where
   conr = transpose
   desr = transpose
...
instance Num a => MVC [[a]] [a] a where
   conc [c] = c
   desc c = [c]
...
instance Num a => Matrix [[a]]

class Vector a

instance Num a => Vector [a]

class Mult a b c where
   (<*>) :: a -> b -> c

instance (Matrix [[a]], Num a) => Mult [[a]] [[a]] [[a]] where
   a <*> b = transpose $ map (\arow -> map (arow <.>) b) $ transpose a

instance (Matrix [[a]], Vector [a], Num a) => Mult [[a]] [a] [a] where
   a <*> b = map (<.> b) $ transpose a

instance (Matrix [[a]], Vector [a], Num a) => Mult [a] [[a]] [a] where
   a <*> b =  map (a <.>) b
{-
instance Blah a where
type M a = [[a]]
-}
{-
instance Num a => MVC [a] [a] where
mapcols = ($)
-}
--maprows :: (MVR [[a]] a1 a, MVR [[b]] b1 b) => ([a] -> [b]) -> a1 -> b1
maprows f = conr . map f . desr

mapcols f = conc . map f . desc

maprows1 f = transpose . map f . transpose

mapcols1 :: ([Double] -> Double) -> [[Double]] -> [Double]
mapcols1 = map

mapcols2 :: ([Double] -> Double) -> [Double] -> Double
mapcols2 = ($)

--a <*> b = maprows (\arow -> mapcols (arow <.>) b) a

class Mult2 a b where
   (|*|) :: a -> b -> b

instance Num a => Mult2 a [[a]] where
   a |*| b = map2 (a *) b
...
gaussAux2 coli rowp row = let
   mult = row !! coli / rowp !! coli
   in
   zipWith (gaussAux3 mult) row rowp

gaussAux2d coli rowp row = let
   mult = row !! coli / rowp !! coli
   in
