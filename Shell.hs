...
searchTree srchStr path =
  let results = filter ((/= []) . snd) $ mapxfx (insertGaps . search srchStr . readFileU) $ fileTree path
   in mapM_
        ( \(fileName, matches) -> do
            putStrLn fileName
            mapM
...
around g contents =
  let (bef, aft) = B.breakSubstring g contents
