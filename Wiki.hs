...
countries m =
   putMap1 $ foldUnion
      $ map (mapGrid "country" . cTextGridH) $ wikiTables m "List of countries by past and projected future population"
...
text1 t = let
   as = findTypes "a" t
   in if null as then extractText t else extractText $ (\(h:_)->h) as
...
set afm ((i, f), c) (fs, rs) = (fs, M.update (Just . Main.replaceElem (afm M.! f) c) i rs)


replaceElem i e x = let
   (a, (_:b)) = splitAt i x
   in a ++ e : b
