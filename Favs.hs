...
joinMapsFuzzy (fl, fi, fr) l r =
   let
      ...
      levs = sort $ concat $ crossWith (\(lk, lv) (rk, rv) -> (levenshtein 3 lk rk, lk, rk, lv ++ rv)) (M.toList l1) (M.toList r1)
...
-}
union3A (i, il, ir) l r =  (if i  then M.union (M.intersection l r) else id) $
                           (if il then M.union (l M.\\ r)           else id) $
                           (if ir then          r M.\\ l            else M.empty)
...
commonSubsequences a b =
   ...
   in reverse $ map reverse $ if null r then rs else r : rs
{-
levenshtein maxDist a b =
   ...
   in c !! length a !! length b
-}

insertmm k v m = case M.lookup k m of
   Just  j -> M.insert k (v:j) m
