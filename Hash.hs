...
repl1 env = do
   ...
   ex1 <- case ex of
      [] -> do
         putStrLn "no parse"
         return $ Value u 0
      ex2:_  -> return $ runDoVars ex2 (map fconstr env)
   putStrLn "print ex"
   print ex
   putStrLn "print ex1"
   print ex1
   putStrLn "unparsing"
   case P.fp expr $ head ex of
      Just j -> putStrLn j
      Nothing -> putStrLn "unparsing failed"
   putStrLn ""
   (res, env3) <- case ex1 of
      Block _ constr exs -> do
         fr <- makeFrame constr
         let env2 = fr : env
         res <- eval2 env2 exs
         return (res, env2)
...
apply1 (f : x : xs) =
   ...
   then do
      case dynApply f $ convertArg1 f x of
         Just j -> do
            k <- case fromDynamic j :: Maybe (IO HashDyn) of
               Just l -> l
               Nothing -> return j
            apply1 (k : xs)
   else apply1a (f : x : xs)

apply1a :: [HashDyn] -> IO HashDyn
apply1a (f : xs) =
   {-
   case dynTypeRep f of
      typeOf (undefined :: Maybe (IORef HashDyn)) ->
      -}
   case fromDynamic f :: Maybe (IORef HashDyn) of
   Just ref -> do putStrLn "readIORef"; dyn <- readIORef ref; apply1 (dyn : xs)
   Nothing -> case fromDynamic f :: Maybe (NamedValue HashDyn) of
      ...
      Nothing -> case fromDynamic f :: Maybe Closure of
         ...
         Nothing -> case fromDynamic f :: Maybe PartApply of
            ...
            Nothing -> do
               case fromDynamic f :: Maybe (Multimethod HashDyn) of
                  Just m -> do
...
myApplyL :: HashDyn -> [HashDyn] -> IO HashDyn
myApplyL f xs = foldM myApply1 f xs

myApply1 :: HashDyn -> HashDyn -> IO HashDyn
...
i77 = toDyn (77 :: Int)
-}
-- convert (args -> IO b) to (args -> IO HashDyn)
...
dynApplyLL :: [HashDyn] -> [HashDyn] -> HashDyn
dynApplyLL fs xs = head $ foldl dynApplyL fs xs

--not used, not sure what it does
dynApplyL2 :: [HashDyn] -> HashDyn -> HashDyn -> HashDyn
...
dynApplyL :: [HashDyn] -> HashDyn -> [HashDyn]
...
-}
env1 =
   [ ("getStr", toDyn getLine)
   , --   ("getAny"  , toDynIO  (parseH <$> getLine)),
   ("putDyn", toDyn putDyn)
   , ("putDynLn", toDyn putDynLn)
   , ("print", toDyn (putDynLn :: HashDyn -> IO ()))
   , ("putStr", toDyn (putStr :: String -> IO ()))
   , (":", toDyn ((:) :: HashDyn -> [HashDyn] -> [HashDyn]))
   , ("[]", toDyn ([] :: [HashDyn]))
   , ("head", toDyn (head :: [HashDyn] -> HashDyn))
   , ("tail", toDyn (tail :: [HashDyn] -> [HashDyn]))
   , ("++", toDyn ((++) :: [HashDyn] -> [HashDyn] -> [HashDyn]))
   , ("map", toDyn mymap)
...
mySet :: IORef HashDyn -> HashDyn -> IO HashDyn
mySet r x = do
   writeIORef r x
   return x

mymap f xs = mapM (\x -> apply [f, x]) (xs :: [HashDyn])
...
eval1 :: Expr -> StateT Env IO HashDyn
...
apply1 (f:xs) =
   ...
   in c
   rd = dynApply1 f xs
   r1 = fromDynamic rd :: Maybe (IO HashDyn)
   in case r1 of
      Just rio -> lift rio
      Nothing  -> return rd
...
niOps1 :: (Typeable n, Num n) => n -> [(VarName, HashDyn)]
