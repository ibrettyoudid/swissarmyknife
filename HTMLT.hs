...
data HTML
   = Tag      { tagType_ :: T.Text, tagAttribs :: [(T.Text, T.Text)], inner :: [HTML] }
...
getRequest :: T.Text -> Request
getRequest = addUserAgent . parseRequest_ . c
--------------------------------------------------------------------------------
getHTTP :: Manager -> T.Text -> T.Text
getHTTP m url = c $ unsafePerformIO $ getHTTPIO m url

--------------------------------------------------------------------------------
getHTTPIO :: Manager -> T.Text -> IO T.Text
getHTTPIO m url = do
...
--lbsofs = LT.pack . map (fromIntegral . ord)

--soflbs = map chr . filter (\x -> x == 10 || x == 13 || x >= 32 && x <= 127 || x >= 160 && x <= 255) . map fromIntegral . LT.unpack

getHTTPRefr :: Manager -> T.Text -> T.Text -> IO T.Text
getHTTPRefr m ref url = do
   nsr <- simpleHTTP m (getRequestRefr (convertString ref) url)
   return $ c $ responseBody nsr

getHTTPReq :: Manager -> Request -> IO T.Text
getHTTPReq m req = do
   let url = reqPath req
...
wget :: Manager -> T.Text -> (T.Text -> Bool) -> (T.Text -> Bool) -> Int -> IO ()
...
formatSpaces :: T.Text -> T.Text
formatSpaces = replace "%20" " "
parseSpaces :: T.Text -> T.Text
...
extractText = trim . squash . clean . mapTree aux map tagText
   where
      ...
      aux tag subtexts  | tagType tag == "ul" = T.concat $ map (++ "\n") subtexts
                        | tagType tag == "td" = T.intercalate " " subtexts
                        | tagType tag == "br" = "\n"
                        | tagType tag == "p"  = "\n" ++ T.intercalate " " subtexts ++ "\n"
...
fetchScripts m html = do
   ...
   --fetcher (mapM brRead scriptbods) $ replicate (length scripturls) LT.empty

fetcher :: (Monad m) => m [B.ByteString] -> [T.Text] -> m [T.Text]
fetcher rs ss = do
   es <- rs
...
htmlP = manyTill (choice [tag, textP, comment, Text <$> takeText]) endOfInput
...
tag = (do
         ...
         let name = smap toLower name1 :: T.Text
         whiteSpace
         attribs <- many attrib
...
ph = BString.putStr . HTMLT.format
...
formatAttrib (name, val) = string7 " " <> fromText name <> string7 "=\"" <> fromText val <> string7 "\""
formatAttribs a = mconcat $ map formatAttrib a
-}
format html = convertString $ toLazyText $ formatLT 0 html :: T.Text

formatHTML = HTMLT.format

formatLT :: Int -> HTML -> Builder
formatLT i (EndTag   typ        ) = mempty --string7 (replicate i ' ') <> "[/" <> fromText typ <> "]\n"
formatLT i (EmptyTag typ attribs) = fromString (replicate i ' ') <> "<" <> fromText typ <> formatAttribs attribs <> "/>\n"
formatLT i (Text            text) = let
   t = squash $ trim text
   
   in if t == "" || t == " " then mempty else fromString (replicate i ' ') <> fromText (trim $ squash text) <> "\n"

formatLT i t@(Tag typ attribs1 terms) = let
   open  = "<"  <> fromText typ <> formatAttribs (attribs $ delAttrib "script" t) <> ">"
   close = "</" <> fromText typ <> ">"
   fterms = map (formatLT (i+3)) terms
   ind = fromString $ replicate i ' '
   --res = open <> mconcat fterms <> close
   
   in if typ == "script" && hasAttrib "script" t
         then 
            ind <> open <> "\n" <> fromText (getAttrib "script" t) <> ind <> close <> "\n"
         else
            ind <> open <> "\n" <> mconcat fterms <> ind <> close <> "\n"

formatAttrib (name, val) = " " <> fromText name <> "=\"" <> fromText val <> "\""
formatAttribs a = mconcat $ map formatAttrib a
...
testParse = parseHTML T.empty file
