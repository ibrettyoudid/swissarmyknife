...
import MHashDynamic3 hiding (Apply, Frame, ValueD, name, tl, value, (!), (==))
...
import System.IO (IOMode (..), SeekMode (..), hClose, hFileSize, hGetChar, hPutStr, hSeek, openBinaryFile)
import System.IO qualified as L
import System.IO.Unsafe
import System.Posix.Files
import System.Process
import Prelude hiding (concat, drop, elem, head, last, length, notElem, null, putStrLn, readFile, span, splitAt, tail, take, writeFile, (!!), (++), (/))
...
m =
  Build
    ...
    ( ArtistK
        <-- upto "/"
        :/ YearK
        <-- Apply (ireadShow :: Iso _ Int) (upto " - ")
        :/ AlbumK
        <-- upto "/"
        :/ TrackK
        <-- upto " - "
...
data Frame
  = FrameHeader {stringID :: NiceText, frameID :: FrameID, frameSize :: Int, tagAltPrsv :: Bool, fileAltPrsv :: Bool, readOnly :: Bool, compression :: Bool, encryption :: Bool, grouping :: Bool, unsyncFr :: Bool, dataLenI :: Bool, frameBytes :: IOString}
...
differences1 strs delims =
  let info = map (infoFromString delims) strs
      tran = transpose1 info
   in case diffcheck tran 0 of
...
matchMetaFS db meta
  | isDir meta =
      let subPs = concatMap (matchMetaFS db . snd) $ M.toList $ subNodes db $ path meta
            ...
            map
              ( \(m, p) ->
                  let ch =
                   ...
                   in ( m,
                        if
                          -- \| length ch >= 2 && fst (ch !! 0) > fst (ch !! 1) * 5 -> [snd (ch !! 0)]
...
matchMetaPath meta =
  let parts = split "/" $ convertString $ path meta
      partsmatch = map (matchMetaString meta) parts
   in partsmatch

-- sortOn snd $ mapMaybe (\case (f, Just i) -> Just (f, i); (f, Nothing) -> Nothing) match
matchMetaString meta str =
  let fss = fieldStrs meta
      fs = map fst fss
      ss = map snd fss
   in mapMaybe
        ( ( \fis ->
              let inField = L.sort $ concat $ zipWith (\(f, i) s -> [i .. i + length s - 1]) fis ss
                  ...
                  f x = case head x of
                    Left c ->
                      let (b, a) = span isLeft x
                       in Left (map (fromLeft '_') b) : f a
                    Right r ->
                      let (b, a) = span isRight x
        ...
        $ crossList
        $ map (\(f, s) -> (Nothing :) $ map Just $ L.findIndices (isPrefixOf s) $ tails str) fss
...
parserFromMatch ms@(m : _) = case m of
  ...
  Right f ->
    ( f <-- case tail ms of
        ...
        (Right r : _) -> error "cannot handle two consecutive fields"
    )
      : parserFromMatch (tail ms)
...
infoFromString1 (delim : ds) str =
  let (word, rest) = split1With (stripPrefix delim) str
   in word : infoFromString1 ds rest
...
updateDB1 bypathold bypathnew =
  let -- join on pathnames
   ...
   in -- give up on the remaining unmatched and just add them
      -- leave out nmdirs
...
combineMPEGFrames totalBytes frs =
  let readBytes = sum $ map mpegFrameBytes frs
   ...
   in [MPEGFrame mVersion mLayer bitRate sampRate totalBytes totalTime empty]
...
tag =
  Build
    ...
    ( Id3K
        ...
        :/ VerMinorK
        <-- int
        :/ (UnsyncK :- ExtHdrK :- ExperiK :- FooterK :- ZK :- ZK :- ZK :- ZK :- ())
        <== Apply tuple8 (bits 8)
        :/ TagSizeK
        <-- int4 0x80
        :/
...
frame (u :- v :- ()) =
  Build
    ...
    ( frameheader v
        ...
        FrameBytesK
        <-- Apply (isync1 (u && v == 4)) (Get FrameSizeK >>== tokens)
...
frameheader 3 =
  StringIDK
    ...
    :/ FrameSizeK
    <-- int4 0x100
    :/ (TagAltPrsvK :- FileAltPrsvK :- ReadOnlyK :- ZK :- ZK :- ZK :- ZK :- ZK :- ())
    <== Apply tuple8 (bits 8)
    :/ (CompressionK :- EncryptionK :- GroupingK :- ZK :- ZK :- ZK :- ZK :- ZK :- ())
    <== Apply tuple8 (bits 8)
    :/ Return ()
frameheader 4 =
  StringIDK
    ...
    :/ FrameSizeK
    <-- int4 0x80
    :/ (ZK :- TagAltPrsvK :- FileAltPrsvK :- ReadOnlyK :- ZK :- ZK :- ZK :- ZK :- ())
    <== Apply tuple8 (bits 8)
    :/ (ZK :- GroupingK :- ZK :- ZK :- CompressionK :- EncryptionK :- UnsyncFrK :- DataLenIK :- ())
    <== Apply tuple8 (bits 8)
    :/ Return ()
...
decodeUCS2 str = case B.take 2 str of
  ...
  _ ->
    if countZeroAlt 0 str >= countZeroAlt 1 str
...
instance P.Frame FrameIDK FrameID Frame where
  ...
  -- myset1 StringIDK value frame = frame { stringID = value }
  myset1 FrameIDK id frame =
    let value = idStringOfFrame id
     in if
          | id `elem` textFrames -> blankText {stringID = value, frameID = id, frameSize = frameSize frame, tagAltPrsv = tagAltPrsv frame, fileAltPrsv = fileAltPrsv frame, readOnly = readOnly frame, compression = compression frame, encryption = encryption frame, grouping = grouping frame, unsyncFr = unsyncFr frame, dataLenI = dataLenI frame, frameBytes = frameBytes frame}
          | id == Txxx -> blankUserText {stringID = value, frameID = id, frameSize = frameSize frame, tagAltPrsv = tagAltPrsv frame, fileAltPrsv = fileAltPrsv frame, readOnly = readOnly frame, compression = compression frame, encryption = encryption frame, grouping = grouping frame, unsyncFr = unsyncFr frame, dataLenI = dataLenI frame, frameBytes = frameBytes frame}
          | id == Uslt -> blankUnsyncLyrics {stringID = value, frameID = id, frameSize = frameSize frame, tagAltPrsv = tagAltPrsv frame, fileAltPrsv = fileAltPrsv frame, readOnly = readOnly frame, compression = compression frame, encryption = encryption frame, grouping = grouping frame, unsyncFr = unsyncFr frame, dataLenI = dataLenI frame, frameBytes = frameBytes frame}
