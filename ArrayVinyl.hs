...
--import Control.Lens hiding (Identity, Const, rmap)
...
class CDimInt a where
   lower :: a -> Int
   upper :: a -> Int
   mult  :: a -> Int

instance CDimInt DimInt where
   lower = diLower
   upper = diUpper
   mult  = diMult

--elemOffset :: (AllConstrained CDimInt rs, RMap rs) => Rec Identity rs -> Rec (Const Int) rs
--elemOffset = rmapMethod (\(Identity field) -> Const $ mult field)

instance CDimInt (DimMap i) where
   lower = dmLower
   upper = dmUpper
   mult  = dmMult
...
--fromElems es = SubArray ((DimInt 0 (l-1) 1) :& RNil) 0 $ A.listArray (0, l-1) es where l = length es

--fromSubs ss 
--elemOffset :: (TupleOf i Int, TupleOf d DimInt) => i -> d -> Int


test = Identity (DimInt 1 2 3) :& Identity (DimInt 4 5 6) :& RNil
...
assocsFromList1 es = zipWith (\x e -> (x :& RNil, e)) [0..] es
assocsFromList2 ess = concat $ zipWith (\x es -> zipWith (\y e -> (x :& y :& RNil, e)) [0..] es) [0..] ess
assocsFromList3 esss = concat $ zipWith (\x ess -> concat $ zipWith (\y es -> zipWith (\z e -> (x :& y :& z :& RNil, e)) [0..] es) [0..] ess) [0..] esss
...
fromList2 zero ess = let
   ...
   in SubArray ((DimInt 0 (d0-1) d1) :& (DimInt 0 (d1-1) 1) :& RNil) 0 $ A.listArray (0, d0 * d1 - 1) $ concatMap (padRWith1 zero d1) ess
...
fromList3 zero esss = let
   ...
   in SubArray ((DimInt 0 (d0-1) (d1*d2)) :& (DimInt 0 (d1-1) d2) :& (DimInt 0 (d2-1) 1) :& RNil) 0 $ A.listArray (0, d0 * d1 * d2 - 1) $ concatMap (padRWith1 zero (d1 * d2) . concatMap (padRWith1 zero d2)) esss
