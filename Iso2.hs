...
nil :: Iso () [alpha]
nil = Iso "nil" f g where
   f ()  =  Just []
   g []  =  Just ()
   g _   =  Nothing

icons :: Iso (alpha :- [alpha]) [alpha]
icons = Iso "icons" f g where
   f (x :- xs) =  Just (x :  xs)
   g (x : xs)  =  Just (x :- xs)
   g _         =  Nothing
  
listCases :: Iso (Either () (alpha :- [alpha])) [alpha]
listCases = Iso "listCases" f g
   where
      f (Left ())        =  Just []
...
inverse (Iso n f g) = Iso ("inverse "++n) g f
...
match c = Iso "match" (\x -> Just c) (\x -> if x == c then Just () else Nothing)

match1 c = satisfy (==c)

total n f g = Iso n (Just . f) (Just . g)

satisfy p = Iso "satisfy" f f where f x = if p x then Just x else Nothing
