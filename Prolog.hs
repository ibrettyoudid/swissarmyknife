...
db = [Apply [Apply [String1 "append", String1 "nil", VarName "Bs", VarName "Bs"]], 
...
-}
run db goal = execStateT (do
   new <- instantiate goal
   doOr db new) $ Env1 M.empty 0
...
unify (Var     a) (Var     b) = do a1 <- getv a
                                   case a1 of
                                       Nothing -> setv a (Var b)
                                       Just a2 -> do b1 <- getv b 
                                                     case b1 of
                                                        Nothing -> setv b a2
                                                        Just b2 -> unify a2 b2
